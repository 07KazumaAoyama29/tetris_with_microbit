...
完成に近づけるためには、"一番下に落ちたらその場所にブロックを固定し、そのブロックの座標をどこかに保存して、次のブロックを出す"ように変更する必要があります。<br>
...
while True:
  ...
  if y < 4:
    y += 1
```
とすると、(2,2)の位置のLEDを点灯させることができます。<br>
これを使えば、一番下に落ちたブロックの位置を記録し、点灯させることができそうです！<br>
### Step2: 一番下に落ちたブロックの座標を保存しよう
現状のプログラムでブロックの落下の部分は、"yの数値が4より小さければ+1、でなければ何もしない"になっています。<br>
これを、"ブロックの一個下の座標が地面ならば今の座標を保存して新しいブロックを作り、でなければyを+1する(落下する)"に変更すればよさそうです。<br>
まずは、現状のif文の条件式を、"yの数値が4より小さければ"から"ブロックの一個下の座標が地面ならば"に変えましょう。<br>
<details><summary>答え</summary>
